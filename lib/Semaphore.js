...
class RedisSemaphore extends RedisMutex {
  ...
  async _refresh(identifier) {
    ...
    if (!refreshed) {
      ...
      console.warn(`[LostLockError] Lost semaphore for key ${this._key}`);
    }
  }

  async acquire() {
    debug(`acquire semaphore (key: ${this._key})`)
    let identifier = await semaphore.acquire(
      this._client,
      this._key,
      this._limit,
      this._lockTimeout,
  ...
  async release(identifier) {
    debug(`release semaphore (key: ${this._key}, identifier: ${identifier})`);
